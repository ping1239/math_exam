...
for (const file of files) {
  ...
  for (let i = 1; i < questions.length; i++) {
    ...
    if (expMatch) {
        if (expMatch[1].trim() === '풀이') {
            console.log(file + ' Q' + id + ' explanation is exactly "풀이"');
        } else if (expMatch[1].trim().length < 40) {
            console.log(file + ' Q' + id + ' has short explanation: ' + expMatch[1].trim());
        }
    }
  }
}
